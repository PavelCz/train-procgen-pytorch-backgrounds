...
if __name__=='__main__':
    ...
    if args.level_seeds_file is not None:
        ...
        print(f'  - Loaded {len(level_seeds_train)} training seeds (container mode)')
        if level_seeds_eval:
            print(f'  - Loaded {len(level_seeds_eval)} eval seeds (sequential mode)')

    def create_venv(args, hyperparameters, is_valid=False):
        # Determine which level seeds to use (if any)
        # Training uses container mode (random draw, cycles forever)
        # Validation uses sequential mode (evaluate each level once)
        if is_valid:
            seeds_to_use = level_seeds_eval
            seeds_mode = 'sequential' if seeds_to_use else None
        else:
            seeds_to_use = level_seeds_train
            seeds_mode = 'container' if seeds_to_use else None
