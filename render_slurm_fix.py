#!/usr/bin/env python
"""
Render script with SLURM/MPI fixes applied before any imports
"""
import sys
import os

# CRITICAL: Set these environment variables BEFORE any other imports
# This must happen before procgen, gym3, or any MPI-related libraries are imported
print("Setting environment variables for SLURM compatibility...")

# Disable OpenMP threading which can conflict with SLURM
os.environ["OMP_NUM_THREADS"] = "1"
os.environ["MKL_NUM_THREADS"] = "1"
os.environ["NUMEXPR_NUM_THREADS"] = "1"
os.environ["OPENBLAS_NUM_THREADS"] = "1"

# Remove all MPI-related environment variables
mpi_vars_to_remove = []
for key in list(os.environ.keys()):
    if any(key.startswith(prefix) for prefix in ['OMPI_', 'PMI_', 'MPI_', 'SLURM_MPI', 'I_MPI_']):
        mpi_vars_to_remove.append(key)
        del os.environ[key]

if mpi_vars_to_remove:
    print(f"Removed MPI environment variables: {', '.join(mpi_vars_to_remove)}")

# Prevent mpi4py from being imported at all
class BlockMPI:
    """Dummy module to prevent mpi4py import"""
...
# Note: This version uses ProcgenEnv (like train.py) which doesn't support
# ViewerWrapper or VideoRecorderWrapper. Use original render.py for those features.
...
if __name__=='__main__':
    ...
    # Check for unsupported features in this SLURM-compatible version
    if not args.noview:
        print("WARNING: --noview is required for this SLURM-compatible version")
        print("         Live viewing is not supported. Exiting.")
        sys.exit(1)
    if args.vid_dir is not None:
        print("WARNING: --vid_dir is not supported in this SLURM-compatible version")
        print("         Video recording is not available. Exiting.")
        sys.exit(1)
    if args.agent_view:
        print("WARNING: --agent_view is not supported in this SLURM-compatible version")
        sys.exit(1)
    ...
    def create_venv_render(args, hyperparameters, is_valid=False):
        ...
        # Note: ProcgenEnv already includes ToBaselinesVecEnv wrapper, so we don't add it again
        
        print("Adding VecExtractDictObs...")
        venv = VecExtractDictObs(venv, "rgb")
        print("✓ VecExtractDictObs added")
        
        normalize_rew = hyperparameters.get('normalize_rew', True)
        if normalize_rew:
            print("Adding VecNormalize...")
            venv = VecNormalize(venv, ob=False)
            print("✓ VecNormalize added")
            
        print("Adding TransposeFrame...")
        venv = TransposeFrame(venv)
        print("✓ TransposeFrame added")
        
        print("Adding ScaledFloatFrame...")
        venv = ScaledFloatFrame(venv)
        print("✓ ScaledFloatFrame added")
